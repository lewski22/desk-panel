...
vi.mock('../api/client', () => ({
  appApi: {
    ...
    checkins:     { manual: vi.fn(), web: vi.fn().mockResolvedValue({}), checkout: vi.fn().mockResolvedValue({}) },
    users:        { list: vi.fn() },
    resources: {
      ...
      update:        vi.fn(),
      remove:        vi.fn(),
      availability:  vi.fn().mockResolvedValue({ slots: [], allDayMode: false }),
      book:          vi.fn().mockResolvedValue({}),
      myBookings:    vi.fn().mockResolvedValue([]),
      cancelBooking: vi.fn().mockResolvedValue({}),
      allBookings:   vi.fn().mockResolvedValue([]),
    },
    bookings:     { cancel: vi.fn(), myList: vi.fn().mockResolvedValue([]) },
