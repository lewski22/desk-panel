...
describe('UsageBar', () => {
  it('renders label and count', () => {
    const view = render(<UsageBar label="Biurka" used={5} limit={10} pct={50} />);
    expect(view.getByText('Biurka')).toBeTruthy();
    expect(view.getByText(/5.*10.*50%/)).toBeTruthy();
  });
  ...
  it('shows warning icon at 80%+', () => {
    const view = render(<UsageBar label="X" used={8} limit={10} pct={80} />);
    expect(view.getByText(/⚠/)).toBeTruthy();
  });

  it('does not show warning below 80%', () => {
    const view = render(<UsageBar label="X" used={7} limit={10} pct={70} />);
    expect(view.queryByText(/⚠/)).toBeFalsy();
  });

  it('renders unlimited when limit is null', () => {
    const view = render(<UsageBar label="X" used={42} limit={null} pct={0} />);
