  ...
  const checkAndProceed = async () => {
    ...
    try {
      ...
      if (!res.available || !res.tenantId) {
        setErr('Logowanie przez Entra ID nie jest skonfigurowane dla tej domeny.');
        setBusy(false);
        return;
      }
      ...
      setErr('Nie można sprawdzić konfiguracji SSO. Sprawdź połączenie.');
    }
    setBusy(false);
  };
  ...
  const loginEntra = async () => {
    ...
    try {
      ...
      if (e.errorCode !== 'user_cancelled' && !e.message?.includes('cancelled')) {
        setErr(e.message ?? 'Logowanie nie powiodło się.');
      }
      setBusy(false);
    }
  };
        ...
        {step === 'email' && (
          <div className="flex flex-col gap-3">
            ...
            <button onClick={checkAndProceed} disabled={busy || !email}
              ...
              {busy ? <span className="inline-flex items-center gap-2 justify-center"><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Sprawdzanie…</span> : 'Dalej'}
            </button>
          </div>
        )}
        {step === 'sso' && (
