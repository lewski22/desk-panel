...
export function UsersPage() {
  ...
  const load = async () => {
    ...
    const [active, deleted] = await Promise.all([
      adminApi.users.list(ORG_ID || undefined).catch(() => [] as any[]),
      adminApi.users.listDeactivated(ORG_ID || undefined).catch(() => [] as any[]),
    ]);
    setUsers(active);
  ...
  const handleEdit = async (e: React.FormEvent) => {
    e.preventDefault(); setBusy(true); setErr('');
    try {
      await adminApi.users.update(target.id, editForm);
  ...
  const handleDeactivate = async () => {
    setBusy(true);
    try {
      await adminApi.users.deactivate(target.id, retDays);
      setModal(null); await load();
    } catch(e:any) { setErr(e.message); }
    setBusy(false);
  };
  ...
  const handleRestore = async (id: string) => {
    try { await adminApi.users.restore(id); await load(); }
    catch(e:any) { alert(e.message); }
  };

  const handleHardDelete = async (id: string, name: string) => {
    if (!confirm(`Trwale usunąć dane konta "${name}"? Aktywności zostaną zachowane.`)) return;
    try { await adminApi.users.hardDelete(id); await load(); }
    catch(e:any) { alert(e.message); }
  };
