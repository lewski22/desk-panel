  ...
  const checkAndProceed = async () => {
    ...
    try {
      ...
      if (!res.available || !res.tenantId) {
        setErr(t('entra.errors.sso_not_configured'));
        setBusy(false);
        return;
      }
      ...
      setErr(t('entra.errors.sso_check_failed'));
    }
    setBusy(false);
  };
        ...
        {step === 'email' && (
          <div className="flex flex-col gap-3">
            ...
            <button onClick={checkAndProceed} disabled={busy || !email}
              ...
              {busy ? <span className="inline-flex items-center gap-2 justify-center"><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />{t('entra.checking')}</span> : t('entra.check')}
            </button>
          </div>
        )}
        {step === 'sso' && (
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 shadow-2xl">
            ...
            <form onSubmit={submit} className="flex flex-col gap-3">
              ...
              <button type="submit" disabled={busy}
                ...
                {busy ? <span className="inline-flex items-center gap-2 justify-center"><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />{t('login.submitting')}</span> : t('login.submit')}
              </button>
            </form>
