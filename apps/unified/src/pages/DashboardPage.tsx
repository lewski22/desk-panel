...
export function DashboardPage() {
  ...
  const methodData   = useMemo(() => (ext?.methods ?? []).map((m: any) => ({
    name:  t(`methods.${m.method}`, m.method),
    value: m._count,
  ...
  const peakHours = useMemo(() =>
    [...(ext?.hourly ?? [])].sort((a: any, b: any) => b.count - a.count).slice(0, 3).map((h: any) => h.hour),
  [ext?.hourly]);
        ...
        <Card className="p-5">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={hourlyFiltered} barCategoryGap="15%">
              <XAxis dataKey="hour" tick={{ fontSize:9, fill:'#a1a1aa' }} axisLine={false} tickLine={false} interval={1} />
              <YAxis hide />
              <Tooltip contentStyle={{ borderRadius:8, border:'1px solid #e4e4e7', fontSize:11 }} cursor={{ fill:'#f9f9f9' }} />
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {desks.map(d => (
              <div key={d.id}
                ...
                style={{
                  background: !d.isOnline ? C_OFFLINE : d.isOccupied ? C_OCCUPIED : d.currentReservation ? C_RESERVED : C_FREE,
