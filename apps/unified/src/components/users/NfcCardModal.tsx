...
export function NfcCardModal({ user, onClose }: Props) {
  ...
  const startScan = async () => {
    ...
    try { await appApi.users.nfcScanStart(user.id); }
    catch (e: any) { setErr(e.message ?? t('users.nfcModal.errors.start_failed')); setMode('choose'); return; }

    pollRef.current = setInterval(async () => {
      try {
        const res = await appApi.users.nfcScanStatus(user.id);
        if (res.status === 'found') {
          stopPolling(); setStatus('found'); setCardUid(res.cardUid ?? ''); setMode('done');
        } else if (res.status === 'timeout') {
          stopPolling(); setStatus('timeout'); setMode('choose');
  ...
  const saveManual = async () => {
    ...
    setBusy(true); setErr('');
    try { await appApi.users.assignCard(user.id, cardUid.trim().toUpperCase()); onClose(); }
      ...
      <div className="flex flex-col gap-4">
        ...
        {mode === 'choose' && (
          <>
            ...
            <button onClick={() => { setMode('manual'); setCardUid(user.cardUid ?? ''); }}
        ...
        {mode === 'scanning' && (
          <div className="flex flex-col items-center gap-4 py-4">
            ...
            <button onClick={() => { stopPolling(); setMode('choose'); setErr(''); }}
